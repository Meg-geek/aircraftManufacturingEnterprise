...
public interface PickerDao extends CrudRepository<Picker, Integer> {
    ...
    @Query(value = "select staff.id, name, surname, brigade_id from worker\n" +
            ...
            "where guild.company_id = :companyId and worker.id IN (select id from pickers)",
            nativeQuery = true)
    List<Picker> getByCompany(@Param("companyId") int companyId);

    @Query(value = "select staff.id, name, surname, brigade_id from worker\n" +
            "        left join brigade on brigade.id = worker.brigade_id\n" +
            "        left join site on brigade.site_id = site.id\n" +
            "        left join staff on staff.id = worker.id\n" +
            "where site.guild_id = :guildId and worker.id IN (select id from pickers);",
            nativeQuery = true)
    List<Picker> getByGuild(@Param("guildId") int guildId);
    ...
    @Query(value = "select staff.id, name, surname," +
            ...
            " and staff.id in (select id from pickers)", nativeQuery = true)
    List<Picker> getFreeWorkers();
}
